	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  /* read data block */
  lat += cp->blk_access_fn(cmd == Fetch ? Fetch : Read, CACHE_BADDR(cp, addr), cp->bsize,
			   repl, now+lat);
	    ...
	    struct cache_blk_t *start)		/* Block to start searching at */
{
  ...
  if (cp->hsize)
  {
    ...
    if  (start) {blk=start->hash_next;}
    else {blk=cp->sets[set].hash[hindex];}
...
cache_getBlock(struct cache_t *cp,		/* cache instance to probe */
	    md_addr_t addr)		/* address of block to probe */
{
  return cache_getNextBlock(cp, addr, NULL);
}
