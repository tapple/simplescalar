	    ...
	    int traceable)		/* traceable system call? */
{
  ...
  switch (syscode)
    ...
    case OSF_SYS_recvfrom:
      {
	socklen_t addr_len;  /*modified by Meng from "int" to "socklen_t"*/
	char *buf;
...
#error No utimes() implementation!
	...
	char *buf;
	word_t osf_addrlen;
	socklen_t addrlen;  /*modified by Meng from "int" to "socklen_t"*/

	/* get simulator memory parameters to host memory */
	mem_bcopy(mem_fn, mem, Read,
		  /* paddrlen */regs->regs_R[MD_REG_A2],
		  &osf_addrlen, sizeof(osf_addrlen));
	addrlen = (socklen_t)osf_addrlen;   /*modified by Meng from "int" to "socklen_t"*/
	if (addrlen != 0)
	  {
